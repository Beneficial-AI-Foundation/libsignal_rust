...
impl GcmGhash {
    ...
    // #[ensures]
    // result.is_ok() ==> result.unwrap().msg_buf_offset == 0
    // #[ensures]
    // result.is_ok() ==> result.unwrap().ad_len == associated_data.len()
    // #[ensures]
    // result.is_ok() ==> result.unwrap().msg_len == 0
